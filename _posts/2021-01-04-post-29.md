...
직관적으로 Feature가 1개인 즉, 2차함수의 꼴($y=\theta_0+\theta_1x$, 우리가 초점을 맞춰야 하는 것은 $\theta$)인 $J(θ)$ 의 최소값을 구할 때 우리는 직관적으로 $J(θ)$ 를 미분하여 극소값을 찾아냅니다. 하지만 실제 문제에서 $θ$ 는 실수가 아니라 n+1차원의 배열이기 때문에 $J(θ)$ 는 $θ_0 .... θ_n$ 벡터의 함수가 됩니다. 
